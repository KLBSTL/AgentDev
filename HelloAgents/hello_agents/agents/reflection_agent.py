...
class ReflectionAgent(Agent):
    def __init__(
        ...
        parsed_templates = dict(prompt_templates or {})
        if system_prompt:
            try:
                maybe_templates = ast.literal_eval(system_prompt)
            except (SyntaxError, ValueError):
                maybe_templates = None

            if isinstance(maybe_templates, dict):
                parsed_templates.update(maybe_templates)
            elif "{task}" in system_prompt:
                initial_prompt_template = initial_prompt_template or system_prompt

        self.initial_prompt_template = (
            initial_prompt_template
            or parsed_templates.get("initial")
            or INITIAL_PROMPT_TEMPLATE
        )
        self.reflect_prompt_template = (
            reflect_prompt_template
            or parsed_templates.get("reflect")
            or REFLECT_PROMPT_TEMPLATE
        )
        self.refine_prompt_template = (
            refine_prompt_template
            or parsed_templates.get("refine")
    ...
    def run(self, task: str):
        ...
        for i in range(self.max_iterations):
            ...
            reflect_prompt = self.reflect_prompt_template.format(
                task=task,
                code=last_code,
                content=last_code,
            )
    ...
    def _get_llm_response(self, prompt: str) -> str:
        ...
        response_text = self.llm.invoke(messages=messages) or ""
...
#     agent = ReflectionAgent(name="ReflectionAgent", llm=llm_client, max_iterations=2)
# 
#     # 3. 定义任务并运行智能体
